...
const Register = () => {
  const { signUp, updateUser, socialLogin } = useContext(AuthContext);
  ...
  const handleSignUp = (e) => {
    ...
    signUp({ name, photoUrl, email, password })
      .then((userCredential) => {
        updateUser(name, photoUrl).then(async (result) => {
          await axios.post("/token", auth.currentUser);
  ...
  return (
    <div className="my-8 flex justify-center flex-row items-center">
      <div className="w-[42%] hidden lg:block mr-8">
        <img
          className="w-full"
          src="https://i.ibb.co/t4TN32n/register.png"
        ></img>
      </div>
